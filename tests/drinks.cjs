...
vm.runInContext(fs.readFileSync(path.join(root,'script-drink.js'),'utf8').replace('return { init, onSessionStart', 'return { audit: {setDrink, DRINKS, DRINK_KEY_TO_RECIPE, SHOP_ID_TO_VISUAL, EQUIP_ID_MAP, getCumulativeSvgContent, buildSyrupMarbling, seededRng}, init, onSessionStart'),context);
